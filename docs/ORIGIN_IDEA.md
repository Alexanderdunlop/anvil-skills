...
| `/research`                                             | `.anvil/`-reading, file-writing-free eighth skill; scoped M3, built M7        | Cut from v1 first, on the grounds that "a prompt to improve this process" is `/feedback`'s output wearing another name. That reasoning was sound, and it applied to a different feature that happened to share the name. What the note actually reaches for is a *contribution channel*, and the distinction is one of direction rather than mechanism: `/feedback` improves **this** repo's files from **this** repo's evidence, while `/research` turns the patterns it could not route into a question aimed **outside** the repo. That is the only path by which evidence from a repo the author will never see reaches the routing table, and it costs a GitHub issue template to build. Restored on that distinction — scoped at M3, the first point `unrouted.md` holds real parked entries to design against, and built at M7, because a contribution channel is worthless until people can install the thing |
