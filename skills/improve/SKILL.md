---
name: improve
description: Read the feedback logs and recommend what should enter or leave the .anvil context files — and change nothing the user has not approved in this run.
disable-model-invocation: true
---

# /improve

Raw log entries in, **recommendations** out. This is the only command that edits
a context file, and it edits nothing without an explicit yes in the same run.

> **Nothing enters a context file and nothing leaves one on your say-so.** You
> read the evidence, work out what it supports, and put it to the user as a
> proposal they accept or reject line by line.

That is not timidity, it is the point. A context file is read on every run of its
command forever, so every line in one should be a line a person chose to pay for.
The alternative is a file that fills with plausible lines nobody agreed to, which
is exactly the failure the budgets exist to prevent — arriving by a different
door.

Every other command appends to the logs and edits nothing.
...
On approval, and only on approval, you may write `CONFIG.md`,
`CRITICAL_PATHS.md`, `REVIEW_RULES.md`, the five `process/*.md`,
`feedback/unrouted.md`, the `status` field of existing log entries, and new
entries appended to `feedback/self.jsonl`.

Approval is per proposal, not a blanket yes for the run. "Do the first two" means
two, and the rest stay proposals for next time.

You may **never** write `BUDGETS.md`, `CLAUDE.md`, any ticket file, or any code.

## 3. Resolve every budget before you propose anything

A budget checked after the fact is a file already over its ceiling, and a
proposal made without one is a proposal the user cannot judge.
...
## 7. Propose the line, and price it

A proposed line is the entry's `proposed_line`: one line, imperative, no
rationale, no URL, no date, no promotion marker, no citation count. The file is
constraint only — the history is in the logs.

Every proposal states four things, because a user cannot judge it otherwise:

- the line, exactly as it would be written
- which file it enters, and that file's count against its ceiling
- the evidence — the `fb-NNNN` ids and how many distinct tickets they span
- what it costs: this line is read on every run of that command from now on

**Under budget.** Propose the append. On approval, write it.

**At budget — one in, one out.** Propose the swap: name the line you would evict,
state why the new one is worth more, and let the user pick either line or
neither. On approval, write the new line, remove the old one, and append the
evicted text to `self.jsonl` as an entry with `"command": "improve"`,
`"category": "unrouted"`, `"status": "parked"`, and the trigger that fired.
Nothing leaves a context file without landing in a log.
...
This is the one case where you have nothing to propose but a choice. Everywhere
else the user is approving your recommendation; here they are making a decision
...
**Trigger 2 — duplication. Proposes a removal.** Read the repo's `CLAUDE.md` and
check whether the substance of any context-file line now appears there. If it
does, show both lines side by side and propose removing the anvil-side one. On
approval, remove it and append the removed text to `self.jsonl` naming trigger 2
— same shape as §7, born `parked` and never re-routed.

Nothing is lost when this fires: the constraint still holds, stated once instead
of twice, and two statements of one constraint is how the two drift apart. It is
still a proposal, because "the substance now appears there" is a judgment about
two sentences that do not match word for word, and a line that quietly stopped
existing is harder to notice than one that quietly appeared.
...
**Trigger 3 — obsolescence. Proposes, on the weakest evidence of the three.** A line promoted more
...
and let the user drop it or keep it. Removing on this evidence without asking is
...
- `open` → `promoted` — the user approved the line and it landed in a context
  file, or trigger 2's removal was approved on the strength of an accepted
  `CLAUDE.md` line
...
Your own appended entries use `"command": "improve"` and the shared `fb-NNNN`
sequence — the next id is the highest in **either** file plus one.

## 11. Idempotence

**Two runs with nothing approved in between change nothing.** A run where the
user approves nothing writes nothing at all — not a line, not a bullet, not a
status flip.

Before proposing any line, check whether it is already in the file. A proposal
the user already accepted, whose `status` you failed to flip, comes back on every
future run — and the second copy of the line looks exactly like a line the file
earned twice.

Re-proposing something the user declined is different. A rejection is not
permanent: evidence accumulates, and a lesson at two sightings the user waved
away may be worth raising again at five. **Say so when you re-raise it** — "you
declined this at two sightings, it is now at five" — so they are deciding again
rather than being asked the same question until they give in.

## 12. The report

End every run with what you propose, then what you did.

**Proposals**, each with its evidence and its price:

- to add — the line, the file, that file's count against its ceiling
- to swap — the new line, the line it would evict, and why it wins
- to remove — the line, and which trigger says so
- to add to `CLAUDE.md` — the line, and why. Yours to propose, never to write
- the conflict, when a file is full and nothing is weaker — both ways out, and
  the decision left with the user

**Then what actually happened:** what was approved and written, what was
declined, and what you rejected before it ever reached the user, naming **which
admission question killed it**.

And every run, whether or not anything was approved:

- parked — count
- **the `unrouted.md` count**, even when it is zero
...
- **write anything to a context file the user did not approve in this run**
- write `BUDGETS.md` or `CLAUDE.md`, or edit any budget key
- ship a context file over its resolved budget
- remove a line — under any trigger — without the user accepting it
...
- treat a broad "looks good" as approval of every proposal in the list. Approval
  is per line, and a user skimming ten proposals has not read ten proposals
...
`/feedback` is the command that puts entries into the logs. You only ever read
them.
